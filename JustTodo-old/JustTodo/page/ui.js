...
function onPageReady() {
    ...
    newTodoInput.addEventListener("contextmenu", (event) => {
        var pasteItem = document.getElementById('new_todo_menu_item_paste')
        document.querySelector('#new_todo_menu_item_paste .text').textContent = lang.text_menu_item_paste
        pasteItem.onclick = (event) => {
            bridge.readClipboard((text) => {
                console.log('readClipboard text=', text)
                newTodoInput.value = text
            })
        }
        document.getElementById('newTodoMenu').show(event)
